...
public extension UIImage {
    ...
    func applySportType(typeSport: String) -> UIImage {
        return UIImage(named: typeSport)!
    }
    ...
    public func applyBlurWithRadius(blurRadius: CGFloat, tintColor: UIColor?, saturationDeltaFactor: CGFloat, maskImage: UIImage? = nil) -> UIImage? {
        ...
        if (size.width < 1 || size.height < 1) {
            print("*** error: invalid size: \(size.width) x \(size.height). Both dimensions must be >= 1: \(self)", appendNewline: false)
            return nil
        }
        if self.CGImage == nil {
            print("*** error: image must be backed by a CGImage: \(self)", appendNewline: false)
            return nil
        }
        if maskImage != nil && maskImage!.CGImage == nil {
            print("*** error: maskImage must be backed by a CGImage: \(maskImage)", appendNewline: false)
            return nil
        }
...
public extension UIImageView{
    ...
    func applyRoundAndBorder() {
        // Rounded profile icon image
        var width = self.frame.size.width
        var height = self.frame.size.height
        var minimun = min(width, height)
        self.layer.cornerRadius = minimun / 2
    ...
    func loadImage(imageURL: String) {
        ...
        var image : UIImage
        var gender: Gender = currentUser.gender
        if gender.description == "" && urlImage != ""{
            loadImageWithUrl(urlImage)
        }else{
            if gender == Gender.MAL{
                image = malePlaceholderImage
            }else{
                image = femalePlaceholderImage
            }
            self.setProperImage(image)
        }
    }
    ...
    func setProperImage(img: UIImage){
        self.image = img
        applyRoundAndBorder()
    }
