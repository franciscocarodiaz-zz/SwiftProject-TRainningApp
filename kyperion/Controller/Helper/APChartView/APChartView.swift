...
@IBDesignable class APChartView:UIView,UIScrollViewDelegate{
    ...
    override func drawRect(rect: CGRect) {
        ...
        for  lineData in collectionLines {
            ...
            if lineData.title == "altimetria" && areaUnderLinesVisible {
                lineData.drawAreaBeneathLineChart()
            }
    ...
    func updateDots(idx: Int, reset: Bool = false){
            var lineData = collectionLines[1]
        if let dotsLayer = lineData.drawDots(UIColor.grayColor(), dotsSelected: lineData.dots) {
            for ll in dotsLayer {
                self.layer.addSublayer(ll)
                self.lineLayerStore.append(ll)
            }
        }
            if let dotsLayer = lineData.drawDots(dotsBackgroundColor, dotsSelected: [lineData.dots[idx]]) {
                for ll in dotsLayer {
                    self.layer.addSublayer(ll)
                    self.lineLayerStore.append(ll)
                }
