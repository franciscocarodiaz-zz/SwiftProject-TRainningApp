...
//
    ...
    func removeDots() -> [CALayer]? {
        var retLayers:[CALayer]? = []
        for currDot in dots {
            currDot.backgroundColor = UIColor.clearColor()
            retLayers?.append(currDot.drawDot(UIColor.clearColor()))
        }
        return retLayers
    }
    
    func drawDots(bgColor:UIColor, dotsSelected:[APChartPoint]) -> [CALayer]? {
        var retLayers:[CALayer]? = []
        for currDot in dotsSelected {
            currDot.backgroundColor = lineColor
            retLayers?.append(currDot.drawDot(bgColor))
        }
        return retLayers
