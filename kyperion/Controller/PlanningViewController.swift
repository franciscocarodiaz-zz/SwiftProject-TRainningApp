...
class PlanningViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, activityManagerDelegate {
    ...
    @IBAction func backButtonPressed(sender: AnyObject) {
        ...
        self.dismissViewControllerAnimated(true, completion: { () -> Void in
            NSNotificationCenter.defaultCenter().postNotificationName(updateTrainingPlanTVDayTableNotificationKey, object: nil)
        })
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        self.navigationItem.title = VC_TRAININGPLAN_VC_TITLE
        self.navigationItem.backBarButtonItem = UIBarButtonItem(title: "<", style:.Plain, target: nil, action: nil)
        self.navigationController?.navigationBar.tintColor = UIColor.mainAppColor()
    ...
    var backButtonItem: UIBarButtonItem {
        return UIBarButtonItem(barButtonSystemItem: .Done, target: self, action: "backButtonPressed:")
    }
    ...
    override func viewDidLoad() {
        ...
        let heightBarFrame = app.statusBarFrame.size.height
        
        // MARK: Create NAV BAR
        navBar.hidden = true
        var x = CGFloat(5.0)
        var y = heightBarFrame
        var width = self.view.frame.width
        var height = CGFloat(30.0)
        var navBarView = UIView(frame: CGRectMake(x, y, width, height))
        navBarView.backgroundColor = UIColor.mainAppColor()
        
        let tapGestureRecognizerLeftButton = UITapGestureRecognizer(target:self, action:Selector("backButtonPressed:"))
        var buttonPrevious = UIButton(frame: CGRectMake(1, 0, height*3, height))
        //buttonPrevious.backgroundColor = UIColor.mainAppColor()
        buttonPrevious.setTitle("< Back", forState: UIControlState.Normal)
        ...
        navBarView.addSubview(buttonPrevious)
        
        self.view.addSubview(navBarView)
        
        // MARK: Create Header view
        var xHeaderView = CGFloat(0.0)
        var yHeaderView = heightBarFrame + CGFloat(height)
        ...
        //planningIcon.backgroundColor = UIColor.greenColor()
        ...
        planningDate.text = currentTrainingPlan.date.dateFormat
        ...
        tableView.frame = CGRectMake(0, headerView.frame.origin.y + headerView.frame.size.height, self.view.frame.width, heightTV)
        //tableView.backgroundColor = UIColor.purpleColor()
        
        print("\n headerView.frame: \(headerView.frame)")
        print("\n planningIcon.frame: \(planningIcon.frame)")
    }
    ...
    */
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        /*
        var cellText : NSString = subtitle
        var cellFont : UIFont = cell.detailTextLabel!.font
        var constraintSize : CGSize = CGSizeMake(CGFloat(280.0), CGFloat(MAXFLOAT))
        let titleFont : UIFont = UIFont(name: FONT_HELVETICA_NEUE, size: 14.0)!
        let attributes = [
            NSFontAttributeName : titleFont,
            NSForegroundColorAttributeName: UIColor.blackColor()
        ]
        var labelSize : CGSize = cellText.sizeWithAttributes(attributes)
        cell.textLabel!.frame.size = labelSize
        cell.detailTextLabel!.frame.size = labelSize
        cell.detailTextLabel?.text = cellText as String
        */
